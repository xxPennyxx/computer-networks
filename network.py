...
#Thanks https://github.com/ppartarr/pyRoute :)
...
def print_routing_table(node):
    ...
    print ("Routing table for {}:".format(node))
...
def check_args(args, command, expected=None, minimum=None, nodes=None, keyword=None, keyword_pos=None):
    if minimum != None and len(args) < minimum:                                                         # check that there is at least min arguments
        print ("{} requires at least  {} arguments - type help for an example".format(command, minimum))
        return False
    if expected != None and len(args) != expected:                                              # check that the expected number of arguments are present
        print ("{} requires {} arguments - type help for an example".format(command, expected))
        return False
    ...
    if nodes != None:                                                                           # check that all nodes given by the user are nodes in the graph
        for node in nodes:
            if node not in G.nodes:
                print ("{} is not a node in the graph - all arguments must be nodes!".format(node))
                return False
    return True
...
def negative_weight_check():
    for edge in G.edges:
        if G[edge[0]][edge[1]]['weight'] < 0:
            print ("Error: graph contains a negative edge between {} and {}".format(edge[0], edge[1]))
            sys.exit(0)
    ...
    if command in ["help","?"]:
        ...
        if check_args(args, command, expected=2, nodes=args):
            ...
            try: 
                ...
                print ("Shortest path between {} and {}is: ".format(src, dest))
                print_path(path)
            except nx.NetworkXNoPath:
                print ("There is no path between {} and {}".format(src, dest))
        command, args = get_input()
    elif command == "cost": 
        ...
        print ("Split horizon has been " + 'enabled') if SPLIT_HORIZON else ('disabled')
        command, args = get_input()
    else:
        if len(command.replace(' ','')) > 0: # command isn't enter or whitespace
            print("{} is not a valid command, type help to display the list of commands".format(command))
        command, args = get_input()
